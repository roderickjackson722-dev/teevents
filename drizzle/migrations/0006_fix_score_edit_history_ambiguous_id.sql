CREATE OR REPLACE FUNCTION public.get_score_edit_history(_tournament_id uuid, _limit integer DEFAULT 200)
 RETURNS TABLE(id uuid, tournament_id uuid, registration_id uuid, player_first_name text, player_last_name text, hole_number integer, old_score integer, new_score integer, edited_by uuid, editor_email text, editor_type text, notes text, created_at timestamp with time zone)
 LANGUAGE plpgsql
...
DECLARE
  _org uuid;
BEGIN
  SELECT t.organization_id INTO _org FROM public.tournaments t WHERE t.id = _tournament_id;
