...
const phases: Phase[] = [
  {
    ...
    features: [
      ...
      { num: 3, name: "Subscription Plan Selection", description: "Two tiers plus Enterprise: Base ($0, 1 active tournament up to 72 players) and Pro ($399 one-time per tournament, unlimited features and players). Enterprise is custom-quoted for white-label, dedicated manager, and SLA. Pro is unlocked per tournament — not org-wide — so organizers only pay for the events that need premium features." },
      ...
      { num: 6, name: "Payout Method Setup", description: "Connect Stripe Connect (automatic payouts), enroll in PayPal bi-weekly transfers, or request paper checks. All revenue routes here automatically via Stripe Connect destination charges — TeeVents never holds organizer funds, and there is no platform reserve hold. A prominent in-dashboard banner explains the standard 2–7 business-day Stripe clearing window so newly onboarded organizers know why their Stripe balance reads $0 immediately after their first charge even though Finances shows the transaction." },
      { num: 7, name: "Notification Preferences & Transaction Emails", description: "Choose which events trigger emails (new registration, sponsorship, donation, auction win, store purchase, refund request) and which addresses receive them. Every paid transaction also fires a celebratory organizer notification email with the gross amount, 5% platform fee, Stripe fee, net payout, and tournament momentum stats — sent to every address in the org's notification settings." },
    ],
  },
  {
    ...
    features: [
      ...
      { num: 14, name: "Pricing & Fee Pass-Through", description: "Set registration price (individual and team), add-ons, and decide whether the golfer pays the 5% platform fee + Stripe processing fees or the organizer absorbs them. Math is shown transparently as a combined line item on the public checkout, and Finances reporting reconciles the actual customer-paid amount against platform fee, Stripe fee, and net to organizer for full audit accuracy." },
  ...
  },
  {
    ...
    features: [
      ...
      { num: 28, name: "Donation Page (Pro)", description: "Public donation page with a live progress bar against your fundraising goal. One-time and recurring options. Tax receipts emailed automatically for nonprofit organizations." },
      { num: 29, name: "Silent Auction & 50/50 Raffle (Pro)", description: "Mobile-friendly bidding portal — bidders register with phone number and place bids from their phone. Buy-Now option, automatic outbid notifications, winner emails, and Stripe checkout for collection." },
      { num: 30, name: "Add-On Store / Merchandise (Pro)", description: "Sell mulligans, skins entry, branded merchandise, dinner tickets, or anything else. Inventory tracking, photos, variants, and seamless add-on at registration checkout. Separate from the platform-run organizer merch shop." },
  ...
  },
  {
    ...
    features: [
      ...
      { num: 49, name: "Post-Event Surveys (Pro)", description: "Automated feedback survey emailed to all participants 24 hours after the event. Star ratings plus open-ended questions. Results aggregated in the dashboard." },
      { num: 50, name: "Finances Dashboard", description: "Full transaction history (registrations, sponsors, donations, auction, store, vendors) with accurate per-row breakdown of customer-paid amount, 5% platform fee, Stripe processing fee, and net to organizer. Reconciles passed-through fees correctly when the golfer covers fees. Revenue by category, paid/unpaid breakdown, budget vs. actual, and CSV export." },
      ...
      { num: 52, name: "Refund Management", description: "Players request refunds via the link in their confirmation email (the public site does not display a refund policy). Organizer reviews each request, approves/declines with one click, and Stripe processes the refund automatically." },
      { num: 53, name: "Stripe Payouts", description: "Net proceeds (registration + fundraising minus 5% TeeVents fee minus Stripe fees) deposited to the organizer's bank account via Stripe Connect destination charges. The first cleared deposit can take 2–7 business days for newly onboarded accounts; subsequent payouts follow the organizer's chosen Stripe schedule." },
