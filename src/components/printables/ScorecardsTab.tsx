...
function scorecardHtml(r: Registration, tournament: Tournament | null) {
  const holes = 18;
  const par = tournament?.course_par ?? 72;
  const parPerHole = Math.round(par / holes);
  ...
  const parCells = Array.from({ length: holes }, () =>
    `<td style="border:1px solid #ccc;padding:4px 6px;text-align:center;font-size:11px;color:#666;">${parPerHole}</td>`
  ).join("");

  const emptyCells = Array.from({ length: holes }, () =>
    `<td style="border:1px solid #ccc;padding:10px 6px;text-align:center;">&nbsp;</td>`
  ).join("");

  return `
    <div style="page-break-inside:avoid;margin-bottom:24px;border:2px solid #1a5c38;border-radius:8px;padding:16px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
        <div>
          ...
          ${tournament?.course_name ? `<div style="font-size:12px;color:#666;">${tournament.course_name} &bull; Par ${par}</div>` : ""}
        </div>
      ...
      ${r.group_number != null ? `<div style="margin-top:8px;font-size:11px;color:#1a5c38;">Starting Hole: ${r.group_number}</div>` : ""}
    </div>`;
}

export default function ScorecardsTab({ tournament, registrations, loading }: Props) {
  const allHtml = registrations.map((r) => scorecardHtml(r, tournament)).join("");
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                {tournament?.course_name && (
                  <p className="text-xs text-muted-foreground">{tournament.course_name} &bull; Par {tournament.course_par ?? 72}</p>
                )}
              </div>
                  ...
                  <tr>
                    {Array.from({ length: 18 }, (_, i) => (
                      <td key={i} className="border border-border px-1.5 py-1 text-center text-muted-foreground">
                        {Math.round((tournament?.course_par ?? 72) / 18)}
                      </td>
                    ))}
                    <td className="border border-border px-1.5 py-1 text-center font-semibold text-foreground">{tournament?.course_par ?? 72}</td>
                  </tr>
                  <tr>
