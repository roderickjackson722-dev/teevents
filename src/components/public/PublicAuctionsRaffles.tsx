...
export function PublicAuctionsRaffles({
  ...
  const reload = async () => {
    const [{ data: a }, { data: r }] = await Promise.all([
      supabase.from("auctions").select("*").eq("tournament_id", tournamentId).order("created_at", { ascending: false }),
      supabase.from("raffles").select("*").eq("tournament_id", tournamentId).order("created_at", { ascending: false }),
    ]);
