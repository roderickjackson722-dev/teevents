import { LeaderboardRenderer, mergeDesign, type LbRow } from "@/components/leaderboard/LeaderboardCore";
...
export default function SampleLeaderboard({ eventName, rows, logoUrl, primaryColor, secondaryColor, compact = false }: Props) {
  const leaderboardRows: LbRow[] = rows.map((row, index) => ({
    ...
    parPlayed: Number(row.thru || 18) >= 18 ? 72 : Math.max(4, Number(row.thru || 1) * 4),
  ...
  return <LeaderboardRenderer compact={compact} design={design} title={eventName} rows={leaderboardRows} logoUrl={logoUrl} subtitle="Live Leaderboard" />;
}