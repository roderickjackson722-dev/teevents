...
// ── Demo Steps ──
const STEPS = [
  { num: 1, title: "Welcome & Quick Overview", minutes: 2, icon: MessageSquare, hero: "In just 25 minutes I'll show you how TeeVents replaces spreadsheets and lets you run a professional golf tournament in minutes.", keyMessage: "Zero upfront cost on Base plan • $5 flat platform fee per transaction • Payments split automatically — we never hold your money", bullets: ["Introduce yourself and thank them for their time.", 'Ask: "Tell me about your tournament — how many players, is it a fundraiser, how do you manage it today?"', "Listen for pain points: spreadsheets, manual check-in, no website, payment headaches.", 'Transition: "Let me show you exactly how TeeVents solves all of that."'], route: null },
  ...
  { num: 5, title: "Registration, Payments & Fees", minutes: 5, icon: CreditCard, hero: "$5 flat TeeVents platform fee + Stripe fee shown clearly • Default = passed to players so you keep 100% of advertised price • Toggle to absorb fees if you want.", keyMessage: "$150 registration example → Golfer pays $159.80 (fees passed) or Organizer nets $140.35 (fees absorbed).", bullets: ["Show Registration page with tiers, group sizes, and checkout breakdown.", "Walk through the fee breakdown: Base Price → $5 Flat Fee → Stripe Fee → Total.", '"You keep 100% of your advertised price by default."', "Show the toggle in Settings to absorb fees instead.", "Mention promo codes, group registration (up to 4 players), and waitlist."], route: "/dashboard/registration" },
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {FLYER_TEMPLATES.map((flyer) => (
          <Card key={flyer.id} className="overflow-hidden">
            {/* Preview */}
            <div className="bg-golf-green-dark p-6 text-primary-foreground min-h-[200px] flex flex-col justify-center">
      ...
      <div className="min-h-screen bg-background">
        <div className="max-w-5xl mx-auto px-4 py-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-display font-bold text-foreground">TeeVents Sales Hub</h1>
              <p className="text-sm text-muted-foreground">Demo scripts, study materials, and marketing assets</p>
            </div>
            <div className="flex gap-2">
              <Button size="sm" variant="outline" onClick={() => navigate("/sales-hub/demo-talk-track")}>📋 Payment Talk Track</Button>
              <Button size="sm" variant="outline" onClick={() => navigate("/sales/demo-agenda")}>📄 Demo Agenda</Button>
            </div>
          </div>
