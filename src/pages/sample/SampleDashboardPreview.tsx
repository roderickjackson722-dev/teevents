...
export default function SampleDashboardPreview() {
  ...
  const totalRevenue =
    participants.length * (sample.registration_fee_cents || 0);
  ...
  return (
    <div className="min-h-screen flex flex-col w-full bg-background">
      ...
      <div
        className="px-4 py-2.5 flex items-center justify-center gap-3 text-sm font-medium z-50 flex-wrap"
        style={{
          backgroundColor: sample.secondary_color || "#F5A623",
          color: sample.primary_color || "#1a5c38",
        }}
      >
        <Eye className="h-4 w-4 flex-shrink-0" />
        <span>
          You're viewing a sample dashboard — this is what your real dashboard
          looks like.
        </span>
        <Link
          to="/get-started"
      ...
      <div className="flex flex-1 min-w-0 w-full">
        ...
        {/* Desktop sidebar */}
        <aside
          className="hidden md:flex flex-col w-64 text-primary-foreground border-r border-border overflow-y-auto max-h-[calc(100vh-44px)] sticky top-[44px]"
          style={{ backgroundColor: sample.primary_color || "#1a5c38" }}
        >
          <SidebarNavContent
            activeNav={activeNav}
        ...
        <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
          <SheetContent
            side="left"
            className="p-0 w-72 bg-primary text-primary-foreground border-r border-border overflow-y-auto"
          >
            <SidebarNavContent
              activeNav={activeNav}
              onSelect={(t) => {
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header
            className="h-16 flex items-center justify-between border-b-2 px-3 sm:px-4 gap-2"
            style={{
              borderColor: sample.secondary_color || "#F5A623",
              backgroundColor: `${sample.secondary_color || "#F5A623"}18`,
            }}
          >
            ...
            <Link
              to={`/sample/${slug}`}
              className="text-xs sm:text-sm text-muted-foreground hover:text-foreground inline-flex items-center gap-1 flex-shrink-0"
            >
          ...
          <div className="md:hidden bg-secondary/10 border-b border-secondary/30 px-3 py-2 flex items-center justify-between gap-2">
            <span className="text-xs text-foreground">
              Explore every organizer section →
            </span>
            <button
              onClick={() => setMobileNavOpen(true)}
          ...
          <main className="flex-1 bg-golf-cream p-3 sm:p-4 md:p-6 overflow-x-auto">
              ...
              <>
                ...
                <div className="bg-card rounded-lg border border-border p-6 mb-6">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <Button onClick={() => setActiveNav("Players & Pairings")}>
                      <Users className="h-4 w-4 mr-2" />
                      Players & Pairings
                    </Button>
                    <Button
                      variant="outline"
                      onClick={() => setActiveNav("Registration Management")}
                    >
                    ...
                    <Button
                      variant="outline"
                      onClick={() => setActiveNav("Live Leaderboard")}
                    >
              ...
              <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Sample preview of the {activeNav} workspace.
                  </p>
                </div>
                <Button
                  variant="outline"
            ...
            {activeNav === "Live Leaderboard" ||
            activeNav === "Live Leaderboard (View)" ||
            activeNav === "Scoring" ? (
              <LeaderboardPanel tournamentName={orgName} />
            ) : null}
            {activeNav === "Sponsorship Management" ||
            activeNav === "Sponsor Management" ? (
              <SponsorsPanel />
            ) : null}
            {activeNav === "Finances" ? <FinancesPanel /> : null}
            {activeNav === "Payout Settings" ? <PayoutPanel /> : null}
            {activeNav === "Share & Promote" ? (
              <SharePanel slug={slug || ""} tournamentName={orgName} />
            ) : null}
            {activeNav === "Check-In" ? <CheckInPanel /> : null}
            {activeNav === "Volunteers" ? <VolunteersPanel /> : null}
            {activeNav === "Email Templates" || activeNav === "Messages" ? (
              <EmailTemplatesPanel />
            ) : null}
            {activeNav === "Auctions" ? <AuctionsPanel /> : null}
            {activeNav === "Raffles" ? <RafflesPanel /> : null}
            {activeNav === "Media Clips" || activeNav === "Photo Gallery" ? (
              <MediaClipsPanel />
            ) : null}
            {activeNav === "Tournament Details" ||
            activeNav === "View Tournament" ? (
              <SiteBuilderPanel
            ...
            {activeNav === "CRM" ? (
              <CrmPanel tournamentName={orgName} participants={participants} />
            ) : null}
...
function GenericFeaturePanel({
  ...
  > = {
    "Planning Guide": {
      description:
        "Step-by-step 30-task checklist to plan your tournament from idea to event day.",
      list: [
        "✓ Set tournament date & venue",
        "✓ Define entry fees and sponsorship tiers",
        "◯ Open registration",
        "◯ Confirm catering",
        "◯ Print scorecards & cart signs",
        "◯ Brief volunteers",
      ],
    },
    "Setup Checklist": {
    ...
    "View Tournament": {
      description: `Opens the public page for ${tournamentName}.`,
    },
    "Course Details": {
    ...
    "Pin Sheets": {
      description: "Auto-generate printable pin sheet PDFs for each round.",
    },
    "Handicap Settings": {
      description: "Configure handicap rules used to compute net scores.",
      rows: [
        { label: "System", value: "USGA" },
        { label: "Course Handicap", value: "100% allowance" },
        { label: "Max Handicap", value: "36.0" },
      ],
    },
    "Tournament Details": {
      description: "Edit name, date, format and fees.",
      rows: [
        { label: "Name", value: tournamentName },
        { label: "Format", value: "Scramble" },
        { label: "Player Entry", value: fmt(25000) },
        { label: "Team Entry", value: fmt(100000) },
      ],
    },
    "Registration Management": {
      description: `${participants.length} registrations received. Manage payments, refunds and team groupings.`,
      rows: [
        { label: "Paid", value: `${participants.length}` },
        { label: "Waitlist", value: "3" },
        { label: "Comped", value: "2" },
      ],
    },
    Lodging: {
      description:
        "Block hotel rooms and let players self-book at your group rate.",
    },
    "Team Management": {
      description:
        "Invite assistants with granular permissions (registration, finances, scoring, etc.).",
    },
    "Organization Info": {
      description:
        "Logo, mission, 501(c)(3) info — used on receipts and public site.",
    },
    "Flyer Studio": {
      description:
        "Generate a print-ready flyer in Canva with your branding pre-filled.",
    },
    Printables: {
      description:
        "Scorecards, cart signs, hole assignments, name badges, sponsor signs — all auto-populated and PDF-ready.",
    },
    "Email Templates": {
      description:
        "Customize confirmation, reminder and thank-you emails sent to participants.",
    },
    "Public Search": {
      description:
        "Toggle whether your tournament appears in the public TeeVents search.",
    },
    Waitlist: {
      description:
        "Automated waitlist with 24-hour claim window when a spot opens.",
      list: [
        "1. Casey Morgan — added 2 days ago",
        "2. Pat Lee — added 1 day ago",
        "3. Drew Kim — added 3 hours ago",
      ],
    },
    "Check-In": {
      description:
        "Scan QR codes on event day to check players in and assign carts.",
    },
    "Tee Sheet": {
      description: "Drag-and-drop pairings and shotgun-start hole assignments.",
    },
    "Test Simulator": {
      description:
        "Simulate scores to verify your leaderboard and scoring rules before event day.",
    },
    Volunteers: {
      description: "Recruit and schedule volunteers by station and shift.",
    },
    Vendors: {
      description: "Sell vendor/exhibitor booth space with custom tiers.",
    },
    "Side Events": {
      description:
        "Sell add-ons: skins, mulligans, raffle tickets, beat-the-pro, etc.",
    },
    "Team Performance": {
      description: "Post-round team stats and trophy graphics.",
    },
    "Event Day Contests": {
      description:
        "Closest-to-the-pin, long drive, hole-in-one — track on-course contests.",
    },
    Messages: {
      description: "Send announcements and updates to all participants.",
      list: [
        "📣 Tee times posted",
        "☀️ Weather update: clear skies",
        "🏆 Awards ceremony 3:30pm in the clubhouse",
      ],
    },
    Budget: {
      description: "Track planned vs actual revenue and expenses.",
      rows: [
        { label: "Projected Revenue", value: fmt(totalRevenue) },
        {
          label: "Projected Expenses",
          value: fmt(Math.round(totalRevenue * 0.45)),
        },
        { label: "Projected Net", value: fmt(Math.round(totalRevenue * 0.55)) },
      ],
    },
    "Add On Store": {
      description:
        "Optional add-ons (mulligans, range balls, raffle bundles) sold during registration.",
    },
    "Director Shop": {
      description:
        "Order branded TeeVents merch — banners, polo shirts, signage — for your event.",
    },
    "Surveys & Feedback": {
      description: "Post-event survey automation. Average rating: ⭐ 4.8 / 5.0",
    },
    "Photo Gallery": {
      description: "Share event photos with players — auto-tagged by team.",
    },
    Donations: {
      description:
        "Accept additional donations on top of registration. To date: " +
        fmt(125000),
    },
    Auctions: {
      description:
        "Live & silent auctions with bidder paddles and online bidding.",
    },
    Raffles: {
      description: "Sell raffle tickets digitally with random winner draw.",
    },
    "Media Clips": {
      description: "Upload short event recap videos for social sharing.",
    },
    "Day-Of Page": {
      description:
        "Mobile landing page for players on event day — schedule, pairings, leaderboard.",
    },
    "General Settings": {
      description:
        "Tournament settings, branding, custom domain, notifications.",
    },
    "Help Center": {
      description: "Step-by-step guides, video walkthroughs, and chat support.",
    },
    "Activity Log": {
      description: "Full audit trail of every change made to your tournament.",
      list: [
        "2h ago — Sponsor 'Albany Auto' added",
        "5h ago — Registration opened",
        "1d ago — Tournament details updated",
      ],
    },
  };

  const c = content[title] || {
    description: `Preview of the ${title} section. With TeeVents this is a full-featured tool — this is a quick mockup of what you'll see.`,
  };
...
function DemoBtn({
  children,
  tip = "Active in the live version",
  ...rest
}: any) {
  return (
    <button
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${styles}`}
    >
      {status}
    </span>
  );
    ...
    <div className="bg-card rounded-lg border border-border p-6 mb-6">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <DemoBtn tip="CSV export available in the live version">
            Export CSV
          </DemoBtn>
        </div>
      </div>
    ...
    <div className="bg-card rounded-lg border border-border p-6 mb-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {MOCK_SPONSORS_FULL.map((s) => (
          ...
          >
            <img
              src={`https://placehold.co/300x140/${s.color}/ffffff?text=${encodeURIComponent(s.name)}`}
              alt={s.name}
    ...
    <div className="bg-card rounded-lg border border-border p-6 mb-6">
      ...
      <div className="flex items-center gap-3 overflow-hidden bg-muted/40 rounded-md p-2 mb-4">
        ...
        {MOCK_SPONSORS_FULL.slice(0, 4).map((s) => (
          <img
            key={s.name}
            src={`https://placehold.co/100x32/${s.color}/ffffff?text=${encodeURIComponent(s.name.split(" ")[0])}`}
            alt={s.name}
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        <h3 className="text-lg font-display font-bold flex items-center gap-2">
          ...
          Live Leaderboard — {tournamentName}
        </h3>
        <div className="flex items-center gap-2">
          <div className="inline-flex rounded-md border overflow-hidden">
            <button
              onClick={() => setView("gross")}
              className={`px-3 py-1 text-xs font-semibold ${view === "gross" ? "bg-primary text-primary-foreground" : "bg-background"}`}
            >
              Gross
            </button>
            <button
              onClick={() => setView("net")}
              className={`px-3 py-1 text-xs font-semibold ${view === "net" ? "bg-primary text-primary-foreground" : "bg-background"}`}
            >
              Net
            </button>
          </div>
          <DemoBtn tip="Auto-refreshes live during tournaments">
            Refresh
          </DemoBtn>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {MOCK_LB.map((l) => (
              <tr
                key={l.pos}
                className={`border-b ${l.pos <= 3 ? "bg-secondary/5" : ""}`}
              >
                <td className="py-2 font-bold">{l.pos}</td>
                <td className="font-medium">{l.name}</td>
                <td
                  className={`text-right ${view === "gross" ? "font-bold text-primary" : ""}`}
                >
                  {formatScore(l.gross)}
                </td>
                <td
                  className={`text-right ${view === "net" ? "font-bold text-primary" : ""}`}
                >
                  {formatScore(l.net)}
                </td>
    ...
    <div className="bg-card rounded-lg border border-border p-6 mb-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        <div className="border rounded-md p-3 bg-primary/5">
          ...
          <div className="text-xl font-bold text-primary">$8,100.00</div>
        </div>
        <div className="border rounded-md p-3">
          <div className="text-xs text-muted-foreground">
            Platform Fees (5%)
          </div>
          <div className="text-xl font-bold text-orange-600">$405.00</div>
        </div>
        <div className="border rounded-md p-3">
          <div className="text-xs text-muted-foreground">Stripe Fees</div>
          <div className="text-xl font-bold text-orange-600">$243.00</div>
        </div>
        <div className="border rounded-md p-3 bg-green-50">
          <div className="text-xs text-muted-foreground">Net to You</div>
          <div className="text-xl font-bold text-green-700">$7,452.00</div>
        </div>
      </div>
      <div className="border rounded-md p-3 mb-4 bg-green-50 flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-muted-foreground">Bank •••• 4242</span>
        </div>
      ...
      <div className="border-2 border-green-500 rounded-lg p-5 bg-green-50">
        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
          <div className="flex items-center gap-3">
            <img
              src="https://placehold.co/60x30/635BFF/ffffff?text=stripe"
              alt="Stripe"
              className="h-7 rounded"
            />
        ...
        <div className="grid sm:grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <div className="font-semibold">Bank •••• 4242</div>
          </div>
          <div>
            <div className="text-xs text-muted-foreground">Payout Schedule</div>
            <div className="font-semibold">Automatic (2–3 business days)</div>
          </div>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <div>
            ...
            <code className="block bg-muted p-2.5 rounded font-mono text-xs break-all">
              {url}
            </code>
          </div>
          <div>
            <label className="text-xs text-muted-foreground">Short link</label>
            <code className="block bg-muted p-2.5 rounded font-mono text-sm">
              teev.vent/{slug}
            </code>
          </div>
          ...
          <div className="pt-2">
            ...
            <div className="flex gap-2">
              <DemoBtn tip="Share to Facebook in live version">
                📘 Facebook
              </DemoBtn>
              <DemoBtn tip="Share to LinkedIn in live version">
                💼 LinkedIn
              </DemoBtn>
...
function CheckInPanel() {
  const checked = MOCK_PLAYERS.filter((p) => p.status === "Checked In").slice(
    0,
    5,
  );
  return (
...
const MOCK_EMAILS = [
  {
    ...
    subject: "You're registered! 🏌️",
    preview:
      "Thanks for registering for our tournament. Here are your details and what to expect on tournament day...",
  },
  {
    name: "Tee Time Reminder",
    subject: "Your tee time is tomorrow",
    preview:
      "Just a friendly reminder — your tee time is tomorrow at 8:12 AM. Please arrive 30 minutes early...",
  },
  {
    name: "Post-Event Thank You",
    subject: "Thanks for playing!",
    preview:
      "What a day! Thank you for joining us. View photos, final leaderboard, and donation totals here...",
  },
];
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="md:col-span-1 space-y-2">
          {MOCK_EMAILS.map((m, i) => (
            <button
              ...
              className={`w-full text-left p-3 rounded-md border transition ${i === selected ? "border-secondary bg-secondary/10" : "hover:bg-muted/40"}`}
            >
        ...
        <div className="md:col-span-2 border rounded-lg overflow-hidden">
          ...
          <div className="p-4 bg-white">
            <div className="h-16 bg-gradient-to-r from-primary to-primary/80 rounded mb-3 flex items-center justify-center text-primary-foreground font-display font-bold">
              Your Tournament
            </div>
            <p className="text-sm text-foreground/80 leading-relaxed">
              {e.preview}
            </p>
            <button className="mt-3 bg-secondary text-secondary-foreground px-4 py-2 rounded text-sm font-semibold">
              View Details
            </button>
          </div>
        </div>
    ...
    <div className="bg-card rounded-lg border border-border p-6 mb-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="border rounded-lg overflow-hidden">
          <img
            src="https://placehold.co/400x200/1a5c38/ffffff?text=Signed+Golf+Memorabilia"
            alt="Auction item"
          ...
          <div className="p-4">
            ...
            <p className="text-xs text-muted-foreground mb-3">
              Authenticated PGA Tour signed flag and ball set.
            </p>
            ...
            <DemoBtn tip="Bids placed live during your event">
              Place Bid
            </DemoBtn>
          </div>
        </div>
        <div className="border rounded-lg overflow-hidden">
          <img
            src="https://placehold.co/400x200/F5A623/ffffff?text=Weekend+Getaway"
            alt="Auction item"
            className="w-full h-40 object-cover"
          />
          <div className="p-4">
            ...
            <p className="text-xs text-muted-foreground mb-3">
              2-night stay + round of golf for 4 at a premier resort.
            </p>
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs text-muted-foreground">Current bid</div>
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-6 mb-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="border rounded-lg overflow-hidden">
          <img
            src="https://placehold.co/400x180/10B981/ffffff?text=50%2F50+Cash+Raffle"
            alt="Raffle"
        ...
        <div className="border rounded-lg overflow-hidden">
          <img
            src="https://placehold.co/400x180/8B5CF6/ffffff?text=Pro+Shop+Bundle"
            alt="Raffle"
            className="w-full h-36 object-cover"
          />
          <div className="p-4">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {clips.map((c) => (
          ...
          >
            <div className="relative">
              <img
                src={`https://placehold.co/320x180/${c.color}/ffffff?text=${encodeURIComponent(c.title)}`}
                alt={c.title}
                className="w-full h-32 object-cover"
              />
              <div className="absolute inset-0 flex items-center justify-center bg-black/30 group-hover:bg-black/50 transition">
                <div className="h-12 w-12 rounded-full bg-white/90 flex items-center justify-center text-primary text-xl">
                  ▶
                </div>
              </div>
            </div>
            <div className="p-3">
              ...
              <div className="text-xs text-muted-foreground">2:14</div>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-6 mb-6">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        ...
        <DemoBtn tip="In the live version, this publishes your site">
          Publish Site
        </DemoBtn>
      </div>
      <div className="grid md:grid-cols-3 gap-4">
        <div className="md:col-span-2 border rounded-lg overflow-hidden">
          ...
          <div className="relative h-56 bg-gradient-to-br from-primary to-primary/70 flex flex-col items-center justify-center text-primary-foreground p-6 text-center">
            ...
            <button className="mt-4 bg-secondary text-secondary-foreground px-5 py-2 rounded font-semibold text-sm">
              Register Now
            </button>
          </div>
        </div>
        <div className="space-y-3">
          <div>
            ...
            <div className="flex gap-2">
              {["#1a5c38", "#F5A623", "#3B82F6", "#8B5CF6", "#EF4444"].map(
                (c) => (
                  <button
                    key={c}
                    className="h-8 w-8 rounded-full border-2 border-white shadow"
                    style={{ backgroundColor: c }}
                    title="Click to change in live version"
                  />
                ),
              )}
            </div>
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-6 mb-6">
      ...
      <p className="text-sm text-muted-foreground mb-3">
        Automated 24-hour claim window when a spot opens.
      </p>
      <ul className="space-y-2">
        {list.map((p, i) => (
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {categories.map((cat) => (
        <div
          key={cat.label}
          className={`${cat.color.split(" ").find((c) => c.startsWith("bg-")) ?? ""} rounded-md mx-1 my-1`}
        >
          <div
            className={`border-l-2 ${cat.color.split(" ").find((c) => c.startsWith("border-l-")) ?? ""} ml-1 pl-2`}
          >
...
function CrmPanel({
  ...
  const tasks = [
    {
      task: "Follow up with Acme Corp about gold sponsorship",
      due: "Today",
      owner: "You",
    },
    {
      task: "Send thank-you email to closed sponsors",
      due: "Tomorrow",
      owner: "You",
    },
    {
      task: "Call First Tee Foundation to confirm logo",
      due: "Fri",
      owner: "Marketing",
    },
  ];
  const auditLog = [
    {
      who: "You",
      what: "Logged call with Sarah Johnson (Acme Corp)",
      when: "2h ago",
    },
    {
      who: "System",
      what: "Email opened by mlee@firsttee.org",
      when: "5h ago",
    },
    {
      who: "You",
      what: "Updated stage for Coastal Realty → Closed Won",
      when: "1d ago",
    },
  ];
  return (
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
          <div>
            <h3 className="text-lg font-display font-bold text-foreground">
              CRM — Prospects & Communications
            </h3>
            <p className="text-sm text-muted-foreground">
              Track sponsors, donors, and partners for {tournamentName}.
            </p>
          </div>
        ...
        <div className="bg-card rounded-lg border border-border p-5">
          ...
          <ul className="space-y-2 text-sm">
            {tasks.map((t) => (
              <li
                key={t.task}
                className="flex items-start gap-2 border-b border-border pb-2"
              >
            ...
            {auditLog.map((l, i) => (
              <li key={i} className="border-b border-border pb-2">
                <p className="text-foreground">
                  <span className="font-semibold">{l.who}</span> — {l.what}
                </p>
