...
CREATE OR REPLACE FUNCTION public.get_public_team_roster(_tournament_id uuid)
 RETURNS TABLE(registration_id uuid, first_name text, last_name text, group_number integer, group_position integer, team_name text, tee_time text, scoring_code text)
 LANGUAGE sql
