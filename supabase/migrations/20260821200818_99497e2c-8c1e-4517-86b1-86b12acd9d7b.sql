GRANT SELECT (pairings_config) ON public.tournaments TO anon;

DROP FUNCTION IF EXISTS public.get_public_team_roster(uuid);

CREATE FUNCTION public.get_public_team_roster(_tournament_id uuid)
 RETURNS TABLE(registration_id uuid, first_name text, last_name text, group_number integer, group_position integer, team_name text, tee_time text, scoring_code text, division text)
 LANGUAGE sql
