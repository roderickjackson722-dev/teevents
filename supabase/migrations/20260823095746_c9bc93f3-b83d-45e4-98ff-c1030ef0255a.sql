...
RETURNS TABLE(
  tournament_id uuid, title text, event_date date, course_name text, start_format text,
  page_config jsonb, pairings_config jsonb, logo_url text, hero_image_url text,
  contact_email text, active_round integer, registration_id uuid, group_number integer,
  starting_hole integer, tee_time text, team_name text, flight_name text,
  first_name text, last_name text, group_position integer
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  WITH t AS (
    ...
    SELECT t.*,
           COALESCE(
             NULLIF(LEAST(GREATEST(COALESCE((t.pairings_config ->> 'publishedRound')::integer, 0), 0), t.round_count), 0),
             (
               SELECT gs FROM generate_series(1, t.round_count) gs
  ...
  SELECT
    c.id, c.title,
    COALESCE(NULLIF(c.pairings_config -> 'byDay' -> ((c.round_number - 1)::text) ->> 'roundDate', '')::date, c.date),
    c.course_name, c.round_start_format, c.pairings_page_config, c.pairings_config,
    c.site_logo_url, c.site_hero_image_url, c.contact_email, c.round_number,
    r.id,
    ...
    CASE
      ...
      ELSE COALESCE(
        ...
        g.tee_time::text, r.tee_time::text
      )
    END,
    g.team_name, ti.tier_name, r.first_name, r.last_name,
...
SET pairings_config = COALESCE(pairings_config, '{}'::jsonb) || jsonb_build_object('publishedRound', 2)
WHERE slug = 'black-mecca-golfers-pro-am' OR custom_slug = 'black-mecca-golfers-pro-am';